            ...
            self.results["vulnerabilities"].append(
                {
                    ...
                    "severity": "medium",
                    "remediation": "Quyidagi headerlarni qo'shing: "
                    + ", ".join([h["header"] for h in missing_headers]),
                }
            )
    ...
    def check_forms_sync(self, response):
        ...
        for form in forms:
            ...
            for input_field in inputs:
                ...
                if (
                    input_type == "password"
                    and not form.get("method", "").upper() == "POST"
                ):
                    form_issues.append(
                        {
                ...
                form_info["inputs"].append({"name": input_name, "type": input_type})
    ...
    def check_xss_vulnerabilities(self, response):
        ...
        for script in scripts:
            if script.string and any(
                payload in script.string
                for payload in ["document.cookie", "localStorage", "sessionStorage"]
            ):
                self.results["vulnerabilities"].append(
                    {
    ...
    def gather_info(self, response):
        ...
        for comment in comments:
            if any(
                ...
                for word in ["todo", "fixme", "password", "username", "admin", "config"]
            ):
